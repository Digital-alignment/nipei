...
import { config, defaultVault } from "@/lib/config";
...
// Robust retry wrapper for Windows file locks & multi-process contention
...
export function resolveVaultRoot(overrideRoot?: string): string {
  ...
  // Return the standard DA path even if it doesn't exist yet on disk
  return path.join(os.homedir(), "Desktop", "DA", "digitalalignment");
}
    ...
    try {
      ...
      await retryWithBackoff(async () => {
        try {
          ...
          // Fallback if cross-device or other non-retryable error
