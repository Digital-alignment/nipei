...
If you run Nipei OS **and** native Hermes (Desktop or CLI) on Windows, you may end up with **two Hermes folders** that don't talk to each other. This guide unifies them safely, and explains how to share one Obsidian memory across all your tools.
...
- **Nipei OS** reads the classic `~/.hermes` → `C:\Users\<you>\.hermes`.

So your profiles, keys, MCPs, kanban, and workspace can end up split across two homes — and Nipei OS features (Hermes Chat, Jarvis, Kanban, Workspace, MCPs, your GLM / game-dev / Fusion profiles) only see the one they're pointed at.

The fix is simple: **make everything use ONE home.**

---

## ✅ The easy way (current Nipei OS): set `HERMES_HOME`

Nipei OS now reads the **`HERMES_HOME`** environment variable — the *same* variable the Hermes CLI uses. So one setting points both at the same folder. No junctions, no moving files.

1. Decide your single home. The simplest is the one the native installer already made: `%LOCALAPPDATA%\hermes`.
2. Set it once (PowerShell), then **restart** your terminal + Nipei OS:
   ```powershell
   setx HERMES_HOME "$env:LOCALAPPDATA\hermes"
   ```
3. Done. The Hermes CLI **and** Nipei OS now read the same `%LOCALAPPDATA%\hermes`.

> Prefer not to use an env var? You can instead set it in Nipei OS's own settings file `~/.nipei-os/config.json`:
> ```json
...
## 🔧 The fallback (older Nipei OS that's still pinned to `~/.hermes`): a junction

If your Nipei OS build doesn't read `HERMES_HOME` yet, make `.hermes` a **junction** pointing at the real folder — both paths then resolve to the same bytes on disk.

1. **Close** Nipei OS, Hermes Desktop, any `hermes` CLI, Kanban.
...
6. **Verify** before deleting the backup: `hermes profile list`, then open Nipei OS and confirm Hermes Chat, Kanban, Workspace, MCPs and your profiles all load.
...
1. **Nipei OS settings → `~/.nipei-os/config.json`** — your vault path, your name, model routing, and now `hermesHome`. The one file that survives every update. Put settings here, never in code.
...
And the key reassurance: **Nipei OS doesn't duplicate your tools' state.** It's a dashboard that drives the *real* native CLIs and reads their *real* folders. So keep using Codex Desktop / Hermes Desktop if you like them — Nipei OS just gives you one screen over the same state. Updates only ever replace the app code; they never touch `~/.nipei-os`, your Hermes home, or your vault.
