...
> 🟢 **Easiest:** open any AI agent in the Nipei OS folder and say *"connect my Google Search Console for the SEO Research tab, walk me through it."* It'll do the steps below with you.
...
4. **Download** the JSON it gives you, and save it as exactly:
   ```
   ~/.nipei-os/gsc-oauth-client.json
   ```
5. **Connect** — in the Nipei OS folder, run:
   ```bash
...
## Good to know
- **Read-only + private** — the tool can only *read* Search Console; it never edits your site or account. The token is cached at `~/.nipei-os/gsc-token.json` (keep it private).
