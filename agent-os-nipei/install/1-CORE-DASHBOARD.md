...
The dashboard works out of the box. If you want to point it at custom folders or tools, it reads an optional config file at `~/.nipei-os/config.json`. There's an example file in this pack (`config.example.json`) — copy it there and edit only if you need to. Most people never touch this.
