...
[ -d "$DIR/node_modules" ] && echo "  $G Dashboard installed" || echo "  $Y Dashboard not installed yet → double-click 'Start Nipei OS.command'"
[ -d "$DIR/.next" ] && echo "  $G Dashboard built" || echo "  $Y Not built yet → 'Start Nipei OS.command' does it on first run"
if curl -s -m 2 -o /dev/null http://localhost:3737; then echo "  $G Dashboard is RUNNING → http://localhost:3737"; else echo "  $Y Dashboard not running right now → double-click 'Start Nipei OS.command'"; fi
