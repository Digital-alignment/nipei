...
const TITLES: Record<string, PageMeta> = {
  "/":            { numeral: "I.",    label: "Nipëi OS · Mission Control", title: "Nipëi OS Mission Control", sub: "Duplo Núcleo enterprise command & governance platform." },
  "/organograma": { numeral: "I·d.",  label: "Estrutura · Organograma", title: "Organograma Global & Squads", sub: "Hierarquia do Duplo Núcleo, lideranças humanas (Pajés, CEO), agentes IA e responsabilidades." },
  "/flow":        { numeral: "I·a.",  label: "Modulo · Nipëi Flow", title: "Nipëi Flow",             sub: "Supply chain, real-time inventory, trazabilidad Mutum, QR code & OCR invoice parser." },
  "/people":      { numeral: "I·b.",  label: "Modulo · Nipëi People", title: "Nipëi People",         sub: "CRM unificado 360º, anamnese médica protegida pelo Firewall Espiritual & IA emocional." },
  "/brain":       { numeral: "I·c.",  label: "Modulo · Nipëi Brain", title: "Nipëi Brain",           sub: "Governança global, Minhas Tarefas por Squad, DRE em tempo real & captura passiva WhatsApp." },
  "/ingestion":   { numeral: "I·e.",  label: "Fase 0 · Vault", title: "Auditor de Ingesta (Vault)", sub: "Zero-hallucination agent for ingesting NotebookLM, Drive, PDFs into Nipëi Master Vault." },
  "/infrastructure": { numeral: "I·f.", label: "Fase 1 · Servicios", title: "Servicios Conectados (VPS & Cloud)", sub: "Hostinger VPS (85.31.61.100), Cloudflare Ecosystem, n8n, Evolution API y llaves SSH." },
  ...
  "/prime":       { numeral: "VIII.", label: "Agent · Prime Agent", title: "Prime Agent", sub: "Prime Intellect's self-improving harness. Every tool is Python it writes into one workspace that stays alive — so it searches your files instead of memorising them. Give it a job, set the gates, and it can't call the work done until the checks pass." },
  ...
  "/memory":      { numeral: "XV.",   label: "Vault · Memory",      title: "Nipëi Memory (Vault & Ingestão)",   sub: "Central de Conhecimento, 60 notas ingestadas no Vault Master, auditoria de vacíos & Omi memories." },
...
const SQUAD_HEADER_META: Record<SquadId, { numeral: string; label: string; title: string; sub: string }> = {
  super_user: {
    numeral: "I.",
    label: "NIPËI OS · MISSION CONTROL",
    title: "Nipëi OS Mission Control",
    sub: "Duplo Núcleo enterprise command & governance platform.",
  },
  squad_1_ceo: {
    numeral: "SQUAD I",
    label: "ESTRATÉGIA & VISÃO INSTITUCIONAL",
    title: "Squad I — Estratégia / CEO",
    sub: "Gestão executiva, roadmap de produtos, OKRs e IA assistente de agenda do CEO.",
  },
  squad_2_mutum: {
    numeral: "SQUAD II",
    label: "PRODUÇÃO & ALDEIA MUTUM",
    title: "Squad II — Produção Mutum",
    sub: "Fichas técnicas, extrativismo sustentável e rastreabilidade de matérias-primas rituais.",
  },
  squad_3_retiros: {
    numeral: "SQUAD III",
    label: "RETIROS & HOSPITALIDADE",
    title: "Squad III — Logística Retiros",
    sub: "Logística Samakey, hospedagem em Serra Grande, dietas sagradas e acompanhamento de participantes.",
  },
  squad_4_vendas_mkt: {
    numeral: "SQUAD IV",
    label: "VENDAS & MARKETING TECH",
    title: "Squad IV — Vendas & Marketing",
    sub: "E-commerce Inî Rau, automação de funil, tráfego pago e campanhas de conversão.",
  },
  squad_5_adm_legal: {
    numeral: "SQUAD V",
    label: "ADMINISTRAÇÃO, LEGAL & FINANCEIRO",
    title: "Squad V — Adm / Legal / Financeiro",
    sub: "Consolidação DRE, emissão de NFe, contratos jurídicos e gestão de centro de custos.",
  },
  squad_6_infra: {
    numeral: "SQUAD VI",
    label: "INFRAESTRUTURA & MANUTENÇÃO",
    title: "Squad VI — Infraestrutura & Manutenção",
    sub: "Ordens de serviço de manutenção, inventário de peças e infraestrutura física.",
  },
  squad_7_instituto: {
    numeral: "SQUAD VII",
    label: "INSTITUTO & RELAÇÃO COM DOADORES",
    title: "Squad VII — Instituto Nipëihu",
    sub: "Certificação de origem épica, veto de blindagem ética e gestão de doadores internacionais.",
  },
};
...
export default function TopBar() {
  ...
  const [activeSquad, setActiveSquad] = useState<SquadId>("super_user");
  useEffect(() => {
    const set = () =>
  ...
  if (pathname === "/apollo" || pathname === "/venu") return null;

  const routeTitle = TITLES[pathname] ?? TITLES["/"];
  const currentMeta =
    pathname === "/" && activeSquad !== "super_user"
      ? SQUAD_HEADER_META[activeSquad]
      : routeTitle;

  return (
    <div className="w-full mb-6 md:mb-8 space-y-6">
      ...
      {/* Row 2: Page Eyebrow, Title & Subtitle */}
      <motion.div
        key={`${pathname}-${activeSquad}`}
      ...
      >
        ...
        <div className="eyebrow">
          <span className="num">{currentMeta.numeral}</span>
          <span className="line" />
          <span className="label">{currentMeta.label}</span>
        </div>

        <h1 className="page-title">{currentMeta.title}</h1>
        <p className="page-subtitle max-w-3xl leading-relaxed">{currentMeta.sub}</p>

        <div className="mt-3 status-meta">
