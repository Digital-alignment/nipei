          ...
          hist.length === 0 ? <div className="text-[12.5px] text-[var(--fg-dimmer)] text-center py-6">No saved conversations yet.</div> : (
            hist.map((h) => (
              <div key={h.id} className="border-b border-[var(--panel-border)]/40 pb-2 mb-1.5">
                ...
                {h.apollo && <div className="text-[13.5px] leading-relaxed"><span className="text-[10px] uppercase tracking-widest mr-2" style={{ color: CYAN }}>VENU</span>{h.apollo}</div>}
              </div>
            ))
          )
        ) : (
          <>
            ...
            {lines.map((l) => (
              <div key={l.id} className="text-[14px] leading-relaxed">
                <span className="text-[10px] uppercase tracking-widest mr-2" style={{ color: l.role === "you" ? "var(--gold)" : CYAN }}>{l.role === "you" ? "You" : "VENU"}</span>
