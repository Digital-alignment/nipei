...
//   Workspace : browse everything ever rendered (Nipei OS + Hermes + Downloads)
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[380px_1fr] gap-4">
      <PromptCard title="Create video · HyperFrames" accent={ACCENT}>
        ...
        <textarea value={prompt} onChange={(e) => setPrompt(e.target.value)}
          placeholder="A 10-second cinematic intro for Nipei OS — fade in title, ambient particles, gold-on-aubergine palette"
          rows={5}
      ...
      <PreviewCard title={openJob ? `Render · ${openJob.job.status}` : (active ? active.slug : "Preview")} accent={ACCENT}>
        {openJob ? (
          ...
          <EmptyState icon={<Film size={28} style={{ color: ACCENT }} />} title="Make a video"
            hint="Describe what you want on the left. Nipei OS scaffolds an HTML composition + immediately runs `hyperframes render`. First render takes 30–90s depending on length." />
        )}
      </PreviewCard>
      ...
      <PromptCard title="Avatar · AI Studio" accent={ACCENT}>
        ...
        <div className="space-y-2 mt-3">
          ...
          <textarea value={text} onChange={(e) => setText(e.target.value)}
            placeholder="Hey everyone, welcome to my new Nipei OS dashboard. This is the live AI-agent command centre I run my whole business through…"
            rows={3}
