...
import { Mic, Send, Zap, Cpu, Radio, Maximize2, X, Newspaper, Target, ListChecks, Trophy, CheckCircle2, TrendingUp, Sparkles, FileText, Brain, Circle, Globe, History, Sun } from "lucide-react";
      ...
      <AnimatePresence>
        {hovered && (
          <motion.div
            ...
            className="absolute bottom-full mb-2.5 left-1/2 -translate-x-1/2 z-50 w-64 p-3 rounded-xl border border-[#22d3ee]/40 bg-[#070c14]/95 backdrop-blur-md shadow-2xl pointer-events-none text-left"
          >
            <div className="text-[11.5px] font-bold tracking-wide text-[#22d3ee] mb-1 flex items-center gap-1.5">
              <span>{title}</span>
            </div>
            <div className="text-[11px] leading-snug text-zinc-300 font-sans">{description}</div>
            <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-2 h-2 rotate-45 border-r border-b border-[#22d3ee]/40 bg-[#070c14]" />
          </motion.div>
        )}
      </AnimatePresence>
    </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// SACRED GEOMETRY MANDALA PORTAL — 6-Pointed Star (Merkaba / Star of David)
// Audio-reactive, 60fps Canvas 2D engine with concentric expanding energy waves.
// ─────────────────────────────────────────────────────────────────────────────
function ArcReactor({ phaseRef, levelRef, size }: { phaseRef: React.MutableRefObject<Phase>; levelRef: React.MutableRefObject<number>; size: number }) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const lvl = levelRef.current; // 0..1 audio level from microphone
      const c = phaseColor(phase);
      const [r, g, b] = hexToRgb(c);
      const rgba = (a: number, overrideHex?: string) => {
        if (overrideHex) {
          const [or, og, ob] = hexToRgb(overrideHex);
          return `rgba(${or},${og},${ob},${a})`;
        }
        return `rgba(${r},${g},${b},${a})`;
      };
      ...
      // 1. Ambient Radial Glow & Energy Field
      const bg = ctx.createRadialGradient(cx, cy, 0, cx, cy, size * 0.48);
      bg.addColorStop(0, rgba(0.12 + lvl * 0.16));
      bg.addColorStop(0.5, rgba(0.04 + lvl * 0.06));
      bg.addColorStop(1, "rgba(0,0,0,0)");
      ctx.fillStyle = bg; ctx.fillRect(0, 0, size, size);

      // 2. Concentric Expanding Energy Waves (Audio-reactive expanding ripples)
      ctx.save(); ctx.translate(cx, cy);
      const waveCount = 5;
      for (let w = 0; w < waveCount; w++) {
        const speed = 0.5 + lvl * 0.8;
        const progress = ((t * speed + w / waveCount) % 1);
        const waveRadius = R * (0.3 + progress * 1.2);
        const waveAlpha = (1 - progress) * (0.15 + lvl * 0.45);
        
        ctx.strokeStyle = rgba(waveAlpha);
        ctx.lineWidth = 1 + (1 - progress) * 2;
        ctx.beginPath();
        ctx.arc(0, 0, waveRadius, 0, Math.PI * 2);
        ctx.stroke();

        // Secondary 6-point wave nodes along expanding ripples
        if (lvl > 0.08) {
          for (let k = 0; k < 6; k++) {
            const ka = (k / 6) * Math.PI * 2 + t * 0.2;
            const nx = Math.cos(ka) * waveRadius;
            const ny = Math.sin(ka) * waveRadius;
            ctx.fillStyle = rgba(waveAlpha * 0.8);
            ctx.beginPath();
            ctx.arc(nx, ny, 1.5 + (1 - progress) * 2, 0, Math.PI * 2);
            ctx.fill();
          }
        }
      }
      ...
      // 3. Outer Sacred Ticks & Flower of Life Petal Ring
      ctx.save(); ctx.translate(cx, cy); ctx.rotate(t * 0.08);
      // Ticks (72 ticks for 6-fold symmetry)
      ctx.strokeStyle = rgba(0.4); ctx.lineWidth = 1;
      for (let i = 0; i < 72; i++) {
        const a = (i / 72) * Math.PI * 2;
        const isMain = i % 12 === 0;
        const r0 = R * 1.34, r1 = R * (isMain ? 1.45 : 1.38);
        ctx.globalAlpha = isMain ? 0.8 : 0.3;
        ctx.beginPath(); ctx.moveTo(Math.cos(a) * r0, Math.sin(a) * r0); ctx.lineTo(Math.cos(a) * r1, Math.sin(a) * r1); ctx.stroke();
      }
      // 6 Sacred Petal Arcs (Flower of Life Outer Ring)
      ctx.lineWidth = 1.5; ctx.strokeStyle = rgba(0.45);
      for (let i = 0; i < 6; i++) {
        const a = (i / 6) * Math.PI * 2;
        const px = Math.cos(a) * (R * 1.25);
        const py = Math.sin(a) * (R * 1.25);
        ctx.beginPath();
      ...
      // 4. Counter-Rotating Geometric Rings
      ctx.save(); ctx.translate(cx, cy);
      // Ring A (Clockwise)
      ctx.rotate(t * 0.15);
      ctx.strokeStyle = rgba(0.6); ctx.lineWidth = 1.8;
      ctx.beginPath(); ctx.arc(0, 0, R * 1.18, 0, Math.PI * 2); ctx.stroke();
      // Ring B (Counter-Clockwise)
      ctx.rotate(-t * 0.3);
      ctx.strokeStyle = rgba(0.4); ctx.lineWidth = 1.2;
      ctx.beginPath(); ctx.arc(0, 0, R * 1.08, 0, Math.PI * 2); ctx.stroke();
      ctx.restore();

      // 5. Audio-reactive Energy Corona Spikes (72 Spikes)
      ctx.save(); ctx.translate(cx, cy);
      ctx.shadowBlur = 10 + lvl * 15; ctx.shadowColor = rgba(0.7);
      const spikes = 72;
      for (let i = 0; i < spikes; i++) {
        const a = (i / spikes) * Math.PI * 2;
        const n = 0.5 + 0.5 * Math.sin(i * 2.3 + t * 4);
        const len = R * (0.05 + (lvl * 0.55 + 0.06) * n);
      ...
      // 6. 🌟 THE 6-POINTED STAR (STAR OF DAVID / MERKABA HEXAGRAM) 🌟
      ctx.save(); ctx.translate(cx, cy);
      const starRotateSpeed = t * 0.2;
      ctx.rotate(starRotateSpeed);

      const starRadius = R * (0.78 + lvl * 0.08); // pulses subtly with voice

      // Function to draw an equilateral triangle
      const drawEquilateralTriangle = (angleOffset: number, colorAlpha: number) => {
        ctx.beginPath();
        for (let i = 0; i < 3; i++) {
        ...
        ctx.closePath();
        ctx.strokeStyle = rgba(colorAlpha);
        ctx.lineWidth = 2.2;
        ctx.stroke();
      };

      // Glow effect for Star of David
      ctx.shadowBlur = 16 + lvl * 20;
      ctx.shadowColor = rgba(0.85);

      // Triangle 1: Pointing Up (North)
      drawEquilateralTriangle(-Math.PI / 2, 0.95);

      // Triangle 2: Pointing Down (South)
      drawEquilateralTriangle(Math.PI / 2, 0.95);

      // Inner Interlaced Hexagram Facets & Sacred Geometry Lines
      ctx.shadowBlur = 6;
      ctx.lineWidth = 1.2;
      ctx.strokeStyle = rgba(0.5);
      
      // Draw inner 6-point inner star / hexagram connections
      ctx.beginPath();
      for (let i = 0; i < 6; i++) {
        const a1 = (i / 6) * Math.PI * 2 - Math.PI / 2;
        const a2 = ((i + 2) / 6) * Math.PI * 2 - Math.PI / 2;
        const x1 = Math.cos(a1) * (starRadius * 0.577); // inner hex radius
        const y1 = Math.sin(a1) * (starRadius * 0.577);
        const x2 = Math.cos(a2) * (starRadius * 0.577);
        const y2 = Math.sin(a2) * (starRadius * 0.577);
        ctx.moveTo(x1, y1);
        ctx.lineTo(x2, y2);
      }
      ctx.stroke();

      // Flower of life seed circles centered on the 6 vertices of Star of David
      ctx.lineWidth = 1;
      ctx.strokeStyle = rgba(0.35 + lvl * 0.25);
      for (let i = 0; i < 6; i++) {
        const angle = (i / 6) * Math.PI * 2;
        const vx = Math.cos(angle) * (starRadius * 0.577);
        const vy = Math.sin(angle) * (starRadius * 0.577);
        ctx.beginPath();
        ctx.arc(vx, vy, starRadius * 0.577, 0, Math.PI * 2);
        ctx.stroke();
      }

      ctx.restore(); ctx.shadowBlur = 0;

      // 7. Pulsing Sacred Core (Central Bindu Sun)
      const coreR = R * (0.24 + lvl * 0.24 + 0.03 * Math.sin(t * 3));
      const cg = ctx.createRadialGradient(cx, cy, 0, cx, cy, coreR);
      cg.addColorStop(0, "rgba(255,255,255,0.98)");
      cg.addColorStop(0.35, rgba(0.9, "#22d3ee"));
      cg.addColorStop(0.7, rgba(0.5, "#34d399"));
      cg.addColorStop(1, "rgba(0,0,0,0)");
      ctx.fillStyle = cg;
      ctx.beginPath(); ctx.arc(cx, cy, coreR, 0, Math.PI * 2); ctx.fill();

      // Core Hexagram Ring around central Bindu
      ctx.save(); ctx.translate(cx, cy); ctx.rotate(-t * 0.35);
      ctx.strokeStyle = rgba(0.75, "#22d3ee"); ctx.lineWidth = 1.6;
      ctx.beginPath();
      for (let i = 0; i < 6; i++) {
        const a = (i / 6) * Math.PI * 2;
        const rx = Math.cos(a) * (R * 0.35);
        const ry = Math.sin(a) * (R * 0.35);
...
export default function ApolloView() {
  ...
  const controls = (
    <div className="flex items-center gap-2.5 flex-wrap justify-center font-sans">
      <ControlItem title="Realtime Voice Core" description="Continuous bidirectional audio with instant voice responses (OpenAI speech-to-speech).">
        <button onClick={() => { const n = !realtime; if (n) { if (wakeOnRef.current) stopWake(); if (liveRef.current) toggleLive(); } setRealtime(n); }}
          ...
          style={{ borderColor: realtime ? CYAN : "var(--panel-border)", color: realtime ? CYAN : "var(--fg-dim)", background: realtime ? "rgba(34,211,238,0.16)" : "transparent", fontFamily: "'Outfit', sans-serif" }}>
          <Zap size={13} className={realtime ? "animate-pulse" : ""} /> Realtime {realtime ? "ON" : "OFF"}
        </button>
      </ControlItem>

      <ControlItem title="Hands-free Live Mode" description="Continuous local microphone detection to talk hands-free without clicking buttons.">
        <button onClick={toggleLive} disabled={realtime}
          className="px-3 h-9 rounded-lg border text-[12px] flex items-center gap-1.5 transition disabled:opacity-30"
          style={{ borderColor: live ? TEAL : "var(--panel-border)", color: live ? TEAL : "var(--fg-dim)", background: live ? "rgba(52,211,153,0.14)" : "transparent", fontFamily: "'Outfit', sans-serif" }}>
          <Mic size={13} className={live ? "animate-pulse" : ""} /> Live {live ? "ON" : "OFF"}
        </button>
      </ControlItem>

      <ControlItem title="Wake Word Activation" description='Active background listening when you say "Venu" out loud.'>
        <button onClick={toggleWake} disabled={realtime}
          className="px-3 h-9 rounded-lg border text-[12px] flex items-center gap-1.5 transition disabled:opacity-30"
          style={{ borderColor: wake ? CYAN : "var(--panel-border)", color: wake ? CYAN : "var(--fg-dim)", background: wake ? "rgba(34,211,238,0.12)" : "transparent", fontFamily: "'Outfit', sans-serif" }}>
          <Radio size={13} className={wake ? "animate-pulse" : ""} /> Wake word {wake ? "ON" : "OFF"}
        </button>
      </ControlItem>

      <ControlItem title="Auto / Agent Mode" description="Auto: Instant answers and fast execution. Agent: Deep task execution using Hermes tools.">
        <button onClick={() => setMode((m) => (m === "auto" ? "agent" : "auto"))}
          className="px-3 h-9 rounded-lg border text-[12px] flex items-center gap-1.5 transition"
          style={{ borderColor: mode === "auto" ? TEAL : "#60a5fa", color: mode === "auto" ? TEAL : "#60a5fa", background: mode === "auto" ? "rgba(52,211,153,0.10)" : "rgba(96,165,250,0.10)", fontFamily: "'Outfit', sans-serif" }}>
          {mode === "auto" ? <><Zap size={13} /> Auto</> : <><Cpu size={13} /> Agent</>}
        </button>
      </ControlItem>

      <ControlItem title="Executive Briefing" description="Generates a daily or weekly executive report grounded in your Obsidian Vault notes.">
        <button onClick={() => runBriefing(briefingRange)} disabled={busy}
          ...
          style={{ borderColor: (briefing || briefingLoading) ? CYAN : "var(--panel-border)", color: (briefing || briefingLoading) ? CYAN : "var(--fg-dim)", background: (briefing || briefingLoading) ? "rgba(34,211,238,0.12)" : "transparent", fontFamily: "'Outfit', sans-serif" }}>
          <Newspaper size={13} /> Briefing
        </button>
      </ControlItem>

      <ControlItem title="Synthetic Voice Selection" description="Select the agent's synthetic voice model (Ash, Onyx, Ballad, Echo).">
        <select value={voice} onChange={(e) => setVoice(e.target.value)}
          className="bg-[rgba(0,0,0,0.3)] border border-[var(--panel-border)] rounded-lg px-2 h-9 text-[12px] text-[var(--fg-dim)] outline-none cursor-pointer"
  ...
  return (
    <div className="max-w-[920px] mx-auto" style={{ fontFamily: "'Outfit', sans-serif" }}>
      {/* Header */}
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight" style={{ fontFamily: "'Outfit', sans-serif" }}><span style={{ color: CYAN }}>Venu</span></h1>
            <div className="text-[12px] text-[var(--fg-dim)] font-mono tracking-wider">ANCESTRAL CONSCIOUSNESS · {phaseLabel} · {clock}</div>
          </div>
        </div>
            ...
            <motion.div initial={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.5 }}
              ...
              <div className="text-center font-mono">
                <motion.div initial={{ letterSpacing: "0.1em", opacity: 0 }} animate={{ letterSpacing: "0.55em", opacity: 1 }} transition={{ duration: 1.1 }}
                  className="text-[20px] font-semibold mb-5" style={{ color: CYAN }}>V E N U</motion.div>
                {[
                  "Ancestral Consciousness",
                  "Establish Yawanawa Echo",
                  "Set up Living Bridge",
                  "Access Spiritual Force",
                  "Portal Open",
                ].map((line, i) => (
                  <motion.div key={line} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 + i * 0.42 }}
                    ...
                    <span>{line === "Portal Open" ? "» PORTAL OPEN" : line + " …"}</span>
                    {line !== "Portal Open" && <span style={{ color: TEAL }}>✓</span>}
                  </motion.div>
                ))}
      ...
      <div className="space-y-2.5 pb-10">
          ...
          {turns.map((t) => (
            <motion.div key={t.id} initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="panel p-3.5">
              <div className="text-[10px] uppercase tracking-widest mb-1.5 flex items-center gap-1.5" style={{ color: t.who === "you" ? "var(--gold)" : CYAN }}>
                {t.who === "you" ? "You" : <><Zap size={10} /> VENU</>}
              </div>
        ...
        {turns.length === 0 && (
          <div className="text-center text-[12.5px] text-[var(--fg-dimmer)] leading-relaxed pt-3">
            Try: <span className="text-[var(--fg-dim)]">&ldquo;Venu, brief me.&rdquo;</span> · <span className="text-[var(--fg-dim)]">&ldquo;Give me my weekly briefing.&rdquo;</span><br />
            <span className="text-[var(--fg-dim)]">&ldquo;Venu, open YouTube.&rdquo;</span> · <span className="text-[var(--fg-dim)]">&ldquo;Build a snake game in my workspace.&rdquo;</span>
          </div>
        )}
      </div>
        ...
        {wall && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <div className="relative flex items-start justify-between p-7 font-mono text-[12px]" style={{ color: CYAN }}>
              <div>
                <div className="text-[16px] tracking-[0.3em] mb-1" style={{ color: "var(--fg)" }}>VENU-<span style={{ color: CYAN }}>NIPËI</span></div>
                <div className="text-[var(--fg-dim)] font-bold tracking-wider">NEURAL INTERFACE · ONLINE</div>
              </div>
                  ...
                  <div className="font-mono text-[12px] space-y-2" style={{ color: "var(--fg-dim)" }}>
                    ...
                    <div className="space-y-2 max-h-[44vh] overflow-hidden">
                      {turns.slice(0, 6).map((t) => (
                        <div key={t.id}><span style={{ color: t.who === "you" ? "var(--gold)" : TEAL }}>{t.who === "you" ? "» YOU  " : "» VENU  "}</span><span style={{ color: "var(--fg)" }}>{t.text}</span></div>
                      ))}
