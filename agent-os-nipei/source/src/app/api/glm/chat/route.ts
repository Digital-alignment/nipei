...
export async function POST(req: Request) {
  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      const messages = [
        { role: "system", content: "You are GLM-5.2, Zhipu's frontier coding model, running inside the Nipei OS. Be concise and direct. When asked to build something, return complete, self-contained code." },
