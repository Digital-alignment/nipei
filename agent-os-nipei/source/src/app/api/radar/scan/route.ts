...
const RADAR_DIR = path.join(os.homedir(), ".nipei-os", "radar");
...
function scanPrompt(now: Date): string {
  ...
  return [
    `You are THE RADAR for ${who} — a creator/founder building with AI agents (Claude Code, Hermes, the Nipei OS, AI automation, SEO).`,
    "Their audience: creators, agency owners and founders building with AI agents (Claude Code, Hermes, Nipei OS, AI automation, SEO).",
    "",
...
async function persist(signals: Signal[], scannedAt: string) {
  ...
  if (VAULT_AI_NEWS) try {
    ...
    if (existsSync(file)) {
      ...
      const header = `# AI News — ${niceDay}\n\n> Auto-logged by The Radar (Nipei OS) — what was breaking on X + the web, each sweep appended below.\n`;
