...
const WP_CONFIG = path.join(os.homedir(), ".nipei-os", "wordpress.json");
...
// leave it out for a clean generic article. Lives in ~/.nipei-os/wordpress.json under "profile".
...
export async function POST(req: Request) {
  ...
  catch { return Response.json({ ok: false, error: "WordPress isn't configured (~/.nipei-os/wordpress.json missing)." }, { status: 503 }); }
