...
// path does). So this route — which only ever answers localhost (Nipei OS
// binds 127.0.0.1) — reads the key from ~/.nipei-os/gemini.env (OUTSIDE the
...
const KEY_FILE = path.join(homedir(), ".nipei-os", "gemini.env");
...
export async function POST(req: Request) {
  ...
  if (!key)
    return NextResponse.json(
      { error: "No Gemini API key found. Add GEMINI_API_KEY=… to ~/.nipei-os/gemini.env, then reload." },
      { status: 400 },
    );
