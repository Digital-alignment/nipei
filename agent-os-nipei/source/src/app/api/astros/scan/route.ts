...
// minted by ~/.nipei-os/astros-youtube-auth.py) → YT_API_KEY env → keyless
// fallback (channel RSS feeds + YouTube search-page scrape). The scan itself never
// needs a key to work.

const ASTROS_DIR = path.join(os.homedir(), ".nipei-os", "astros");
...
function synthesisPrompt(videos: RawVideo[], cfg: AstrosConfig, now: Date): string {
  ...
  return [
    ...
    `${who} runs an AI/AI-agents YouTube channel (Claude Code, Hermes, Nipei OS, AI automation, AI SEO, make-money-with-AI). Audience: creators, agency owners, founders building with AI.`,
    `TODAY IS ${today}.`,
    "",
...
async function persist(topics: AstroTopic[], engine: string, videosScanned: number, scannedAt: string) {
  ...
  if (VAULT_DIR) try {
    ...
    if (existsSync(file)) await appendFile(file, block, "utf8");
      ...
      await writeFile(file, `# YouTube Astros — ${niceDay}\n\n> Auto-logged by Hermes Astros (Nipei OS) — what's breaking on YouTube in your niche, each sweep appended below.\n` + block, "utf8");
    }
