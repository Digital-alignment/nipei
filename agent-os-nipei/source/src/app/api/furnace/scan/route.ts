...
const FURNACE_DIR = path.join(os.homedir(), ".nipei-os", "furnace");
...
async function runScan(): Promise<void> {
  ...
  if (!CHANNELS.length) {
    await writeStatus({ running: false, startedAt, endedAt: new Date().toISOString(), error: 'No channels configured — set "youtubeChannel" (or "furnaceChannels") in ~/.nipei-os/config.json, then scan again.' });
    return;
  }
