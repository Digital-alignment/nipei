...
export async function POST(req: Request) {
  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      const messages = [
        { role: "system", content: "You are Sakana Fugu Ultra, running inside the Nipei OS. Behind your reply sits a multi-agent panel of frontier models that vote and synthesise consensus. Be decisive and concrete — give the verdict, not a menu of options. Flag where the panel disagreed or where a claim is unverified." },
