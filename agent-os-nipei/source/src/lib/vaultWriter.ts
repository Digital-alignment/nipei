...
export const AGENTIC_DIR = VAULT_ROOT ? path.join(VAULT_ROOT, "Nipei OS") : "";
...
// ─── MEMORIES ───────────────────────────────────────────────────────
// One file per day under Nipei OS/Memories/YYYY-MM-DD.md
...
export async function listMemoryDays(limit = 14): Promise<{ date: string; path: string }[]> {
  try {
    ...
    const days = items
      ...
      .reverse()
      .slice(0, limit);
    return days.map((d) => ({ date: d, path: `Nipei OS/Memories/${d}.md` }));
  } catch { return []; }
}

// ─── GOALS ──────────────────────────────────────────────────────────
// Single file: Nipei OS/Goals.md with task-list checkboxes
...
// ─── JOURNAL ────────────────────────────────────────────────────────
// One file per day: Nipei OS/Journal/YYYY-MM-DD.md
