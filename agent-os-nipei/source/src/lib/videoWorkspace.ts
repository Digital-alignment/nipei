...
//
// Bucket roots:
//   ~/.nipei-os/video-projects/             — projects created via this UI
...
const PROJECTS_ROOT = path.join(HOME, ".nipei-os", "video-projects");
    ...
    async function walk(dir: string, depth: number, base: string) {
      ...
      for (const it of items) {
        ...
        if (it.isDirectory()) {
          ...
          if (!st) continue;
          // Build preview URL — if file is inside ~/.nipei-os/video-projects use
          // the `project` mode (containment-checked + cleaner). Otherwise use
          // `local` mode (which has its own allowlist).
          const projectsRoot = path.join(HOME, ".nipei-os", "video-projects");
          let url: string;
