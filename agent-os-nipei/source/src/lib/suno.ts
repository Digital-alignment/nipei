// Suno client — reads API key from ~/.nipei-os/suno.env (chmod 600).
...
const KEY_FILE = path.join(HOME, ".nipei-os", "suno.env");
...
export async function generateMusic(opts: GenerateMusicOpts): Promise<{ taskId: string }> {
  ...
  if (!key) throw new Error("Suno API key not configured — write to ~/.nipei-os/suno.env");
