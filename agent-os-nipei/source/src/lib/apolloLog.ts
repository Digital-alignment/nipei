...
//   1. ~/.nipei-os/apollo-conversations.jsonl  — append-only, powers the
//      transcript history in the UI (survives browser clears / tab switches).
//   2. <vault>/Nipei OS/Apollo/YYYY-MM-DD.md   — a human-readable daily note
...
const STATE_DIR = path.join(os.homedir(), ".nipei-os");
...
async function appendToVault(you: string, apollo: string, ts: number): Promise<void> {
  ...
  try {
    ...
    const dir = path.join(VAULT_ROOT, "Nipei OS", "Apollo");
