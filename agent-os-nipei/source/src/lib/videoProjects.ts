// Video project + render job state — for the /video tab in Nipei OS.
//
// HyperFrames projects scaffold under ~/.nipei-os/video-projects/<slug>/
...
export const VIDEO_ROOT = path.join(HOME, ".nipei-os", "video-projects");
export const RENDER_JOBS_FILE = path.join(HOME, ".nipei-os", "video-render-jobs.json");
export const RENDER_LOGS_DIR = path.join(HOME, ".nipei-os", "video-render-logs");
...
export async function listProjects(): Promise<HFProject[]> {
  ...
  for (const e of entries) {
    ...
    const meta = path.join(cwd, "nipei-os.meta.json");
    if (existsSync(meta)) {
...
export async function createProject(prompt: string, customSlug?: string): Promise<{ slug: string; cwd: string }> {
  ...
  await writeFile(path.join(cwd, "nipei-os.meta.json"), JSON.stringify({
    prompt, createdAt: Date.now(), source: "nipei-os/video",
  }, null, 2));
  return { slug, cwd };
}
