...
//   2. ~/.nipei-os/config.json (user override)
...
export interface AgenticConfig {
  ...
  // Obsidian vault root (where Nipei OS writes goals, journal, memories)
  ...
  // to point Nipei OS at a single shared home (e.g. %LOCALAPPDATA%\hermes on Windows).
  ...
  // "userName" in ~/.nipei-os/config.json (or AGENTIC_OS_USER_NAME) to use your name.
  userName: string;

  // The member's own YouTube channel URL (for the Thumbnail Studio research). Blank by
  // default → research stays topic-only. Set "youtubeChannel" in ~/.nipei-os/config.json.
...
function which(cmd: string): string | null {
  if (process.platform === "win32") {
    try {
      ...
      const firstLine = out.split(/\r?\n/).map(l => l.trim()).find(l => l.length > 0 && existsSync(l));
      if (firstLine) return firstLine;
    } catch { /* fallback to path walk */ }
    ...
    for (const dir of pathDirs) {
      ...
      for (const ext of ["", ...pathExts]) {
        ...
        } catch { /* ignore bad path segments */ }
      }
    }
    return null;
  }
...
function loadFileConfig(): Partial<AgenticConfig> {
  const candidates = [
    ...
    path.join(os.homedir(), ".nipei-os", "config.json"),
    path.join(process.cwd(), "nipei-os.config.json"),
...
// The Hermes home — every Nipei OS feature that reads Hermes state (profiles, .env,
...
// Hermes CLI's own contract plus an Nipei OS config override:
...
//   2. "hermesHome" in ~/.nipei-os/config.json
