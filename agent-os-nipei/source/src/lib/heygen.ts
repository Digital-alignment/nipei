// HeyGen client — reads API key from ~/.nipei-os/heygen.env (chmod 600).
...
const KEY_FILE = path.join(HOME, ".nipei-os", "heygen.env");
const CACHE_DIR = path.join(HOME, ".nipei-os", "heygen-cache");
...
export async function listAvatars(): Promise<HeyGenAvatar[]> {
  ...
  if (!key) throw new Error("HeyGen API key not configured — write to ~/.nipei-os/heygen.env");
