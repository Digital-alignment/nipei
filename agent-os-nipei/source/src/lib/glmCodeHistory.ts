...
//   1. ~/.nipei-os/glm-code/history.jsonl  — durable machine history
//   2. <vault>/Nipei OS/GLM Code Log.md    — a human log inside your Obsidian vault
import { appendFile, mkdir, readFile, writeFile } from "node:fs/promises";
import os from "node:os";
import path from "node:path";
import { VAULT_ROOT, AGENTIC_DIR, todayISO, nowHM } from "@/lib/vaultWriter";

const HIST_DIR = path.join(os.homedir(), ".nipei-os", "glm-code");
...
async function appendToObsidian(e: GlmHistoryEntry): Promise<void> {
  ...
  try {
    ...
    const header = "---\ntitle: GLM Code Log\n---\n\nEvery build made with **GLM Code** — Claude Code's agentic harness running on GLM-5.2 (via Ollama Cloud) inside the Nipei OS. Auto-logged from the GLM Code terminal.\n";
