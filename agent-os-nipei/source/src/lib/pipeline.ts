...
// `Nipei OS/Pipeline/items/` so it's visible + editable in Obsidian, isolated
...
export async function listItems(): Promise<PipelineItem[]> {
  ...
  for (const n of names) {
    ...
    try {
      ...
      it.vaultPath = `Nipei OS/Pipeline/items/${n}`;
      out.push(it);
    } catch {}
  }
...
export async function readItem(slug: string): Promise<PipelineItem | null> {
  ...
  try { const raw = await readFile(f, "utf8"); const it = parseItem(raw, slug); it.vaultPath = `Nipei OS/Pipeline/items/${slug}.md`; return it; } catch { return null; }
}
