...
//   1. ~/.nipei-os/opencode/history.jsonl        — durable machine history
//   2. <vault>/Nipei OS/opencode Log.md          — a human log in your Obsidian vault
import { appendFile, mkdir, readFile, writeFile } from "node:fs/promises";
import os from "node:os";
import path from "node:path";
import { VAULT_ROOT, AGENTIC_DIR, todayISO, nowHM } from "@/lib/vaultWriter";

const HIST_DIR = path.join(os.homedir(), ".nipei-os", "opencode");
...
async function appendToObsidian(e: OpenCodeHistoryEntry): Promise<void> {
  ...
  try {
    ...
    const header = "---\ntitle: opencode Log\n---\n\nEvery build made with **opencode** — the open-source terminal coding agent (opencode.ai) running headless inside the Nipei OS. Auto-logged from the opencode terminal.\n";
