...
// (Nipei OS's take on Claude Code "Artifacts": turn a build into a page you can
...
export const PUBLISHED_DIR = path.join(HOME, ".nipei-os", "published");
const MANIFEST = path.join(PUBLISHED_DIR, "manifest.json");
const LOOP_BUILDS = path.join(HOME, ".nipei-os", "loop-builds");
const CLAUDE_PROJECTS = path.join(HOME, ".nipei-os", "claude-projects");
...
export function artifactSite(): ArtifactSite | null {
  try { return JSON.parse(readFileSync(path.join(HOME, ".nipei-os", "artifacts-site.json"), "utf8")); } catch { return null; }
}
...
<title>Nipei OS · Shared Artifacts</title>
<style>
  ...
  <p class="sub">Built with Nipei OS · ${items.length} published</p>
  <div class="grid">
...
export async function publish(id: string, customTitle?: string): Promise<{ ok: boolean; item?: PublishedItem; error?: string }> {
  const site = artifactSite();
  if (!site) return { ok: false, error: "Artifacts site not configured (~/.nipei-os/artifacts-site.json)." };
