...
cp -R . ~/Agentic\ OS/nipei-os
cd ~/Agentic\ OS/nipei-os
...
mkdir -p ~/.nipei-os
cp nipei-os.config.example.json ~/.nipei-os/config.json
# Then edit ~/.nipei-os/config.json with your vault path + agent binary paths
...
Nipei OS reads config from (in priority order):

1. Environment variables
2. `~/.nipei-os/config.json`
...
Put your settings in **`~/.nipei-os/config.json`** — it lives **outside** the app folder, so an update never touches it. Set your vault path there:
...
> ⚠️ **Don't keep keys or config in a `.env` / `.env.local` inside the app folder as your only copy.** The updater now preserves them if they're there, but `~/.nipei-os/config.json` is the update-proof home — use it.
...
Your CLI isn't on `PATH` or auto-detection missed it. Edit `~/.nipei-os/config.json` and paste the full path from `which <agent>`.
