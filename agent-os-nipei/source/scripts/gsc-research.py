...
impressions), and prints a single JSON object on stdout for the Nipei OS
SEO Content Pipeline "Research" tab.

Usage:  gsc-research.py <site-domain> [days] [seed words...]
Auth:   reuses the cached OAuth token at ~/.nipei-os/gsc-token.json
...
def creds():
    if not TOKEN.exists():
        die("No GSC token. Run: python3 ~/.nipei-os/gsc-report.py  (authorizes once)")
    ...
    if not c.valid:
        if c.expired and c.refresh_token:
            ...
            die("GSC token expired. Run: python3 ~/.nipei-os/gsc-report.py  to re-auth")
    return c
