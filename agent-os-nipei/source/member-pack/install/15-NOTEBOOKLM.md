...
The **Notebook** tab connects your dashboard to **Google NotebookLM** — so you can browse your notebooks, add sources, chat with them, and generate audio overviews, videos, mind maps, slides, flashcards and reports, all from inside the Nipei OS.
...
You do **not** need to edit any paths. The dashboard auto-detects `notebooklm-mcp` on your machine (via your PATH and the common install spots). If you installed it somewhere unusual and the tab can't find it, set the full path as `nlmBin` in `~/.nipei-os/config.json` and restart — but for normal installs you'll never need to.
