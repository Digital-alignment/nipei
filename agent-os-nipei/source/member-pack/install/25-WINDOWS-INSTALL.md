# 25 · Install Nipei OS on Windows (Start Here for Windows)

**Yes — Nipei OS runs great on Windows.** The dashboard is just Node + your browser, so every tab and feature works the same as on a Mac.

There's only **one** difference: the double-click `Start Nipei OS.command` files are **Mac-only**. On Windows you start it with one of the two easy paths below. Pick either.
...
Also unzip the `nipei-os` pack somewhere easy to find (like your Desktop) if you haven't already.
...
1. Open your AI coding agent **inside the unzipped `nipei-os` folder**.
2. Paste this one sentence:

   > **"Read SETUP-WITH-AI.md and set up the whole Nipei OS for me on Windows, step by step. Ask me whenever you need a key or a decision."**
...
- **Updating** on Windows: use **`UPDATE-WITH-AI.md`** (your agent updates it for you on any OS), or the PowerShell steps in `UPDATE.md`. The `Update Nipei OS.command` double-click is Mac-only.
...
- **Two Hermes folders / split state?** If you run both Nipei OS and native Hermes, see **`24-WINDOWS-STATE-AND-MEMORY.md`** to unify them with one `HERMES_HOME` setting (advanced — only if you hit it).
