...
Nipei OS launches the `claude` CLI but tells it to talk to your **local Ollama** (`http://localhost:11434`), which proxies the `glm-5.2:cloud` model up to Ollama Cloud. So it's the Claude Code experience on a cheaper brain.
...
3. **An Ollama Cloud account** — because `glm-5.2:cloud` runs in the cloud. Make a free account at **<https://ollama.com>**, then give Nipei OS your key:
   - Get your key from your Ollama account (ollama.com → your settings → API keys).
   - Set it as **`OLLAMA_API_KEY`** (in your environment, or in `~/.nipei-os/config.json`-adjacent `.env.local`).
...
4. Every build is also logged to your Obsidian vault (`Nipei OS/GLM Code Log.md`) if your vault is connected.
...
2. **Log in** (opens a browser; free trial, your account):
   ...
   qoder login
   ```
3. **Create the Qwen 3.8 wrapper.** Nipei OS runs a command called `qoder-qwen` that pins Qoder to the Qwen 3.8 model — make it once:
   ```bash
   mkdir -p ~/.local/bin
...
- **Costs are yours** — Ollama Cloud bills your account for what you generate. Nothing is charged by Nipei OS.
