...
After you install one, just log in to it once the normal way (each tool tells you how). Then its tab in the Nipei OS works.
...
```bash
mkdir -p ~/.nipei-os
echo 'GEMINI_API_KEY=your_key_here' > ~/.nipei-os/gemini.env
chmod 600 ~/.nipei-os/gemini.env
```
...
1. Open (or create) the file `~/.nipei-os/config.json`
