...
> 🧠 **Pro tip for agent-assisted maintenance:** copy these `install/` docs into a folder in your **Obsidian vault** (e.g. `Nipei OS/Install/`) and keep them current after each update. Because the OS reads your vault, your agents (Jarvis, Hermes, the Agent Room) can then pull these docs as context when you ask them to troubleshoot or maintain the OS — so "fix my Memory tab" or "why won't Antigravity connect?" is answered against the real setup steps, not guesswork. *(Re-copy them after each update so they stay accurate.)*
