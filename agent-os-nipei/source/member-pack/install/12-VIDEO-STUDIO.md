...
- **Workspace** — browse every video you've made (from the Nipei OS, Hermes, or your Downloads).
...
> Nothing to buy, no key. It's the same engine the Nipei OS uses to make its own promo videos.
...
2. Save the key in its own file so it stays private:
   ```bash
   mkdir -p ~/.nipei-os
   echo 'HEYGEN_API_KEY=your_key_here' > ~/.nipei-os/heygen.env
   chmod 600 ~/.nipei-os/heygen.env
   ```
