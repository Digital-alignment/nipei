...
The **Kimi Code** tab adds **Kimi K2.7** — Moonshot's coding agent — as another brain in your Nipei OS. Ask it to build something, fix code, or answer a question; it chats and writes files into its own workspace, right alongside Claude, Codex and the rest.
...
You don't edit any paths. The dashboard auto-detects `kimi` on your machine (your PATH and the standard `~/.kimi-code/bin` location). If you installed it somewhere unusual and the tab can't find it, set the full path as `kimi` in `~/.nipei-os/config.json` and restart — but for normal installs you'll never need to.
...
## Good to know
- Kimi builds land in `~/.nipei-os/kimi-projects/` — nothing gets lost.
