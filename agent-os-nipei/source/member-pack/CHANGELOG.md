...
## 2026-07-17
- 🛑 **Clearer start:** a note up top that Nipei OS is a **desktop app you run on your own computer** — don't upload the zip into the Claude/ChatGPT website chat (it won't run there, and Claude won't copy a paid product's code). Unzip it and double-click `Start Nipei OS.command`.
- ⚡ **Latest build refreshed.** Same tabs, newest under-the-hood fixes bundled — double-click `Update Nipei OS.command` to pull it in (keeps all your settings, keys, and notes).
...
- 🔒 **Updates no longer touch your keys or config — this was a real bug, now fixed.** All four update paths (the `.command`, the Windows steps, and the AI updater) now explicitly protect `.env`, `.env.local` and `nipei-os.config.json` from being deleted during an update. We also point you to `~/.nipei-os/config.json` (which lives *outside* the app folder and is never touched). Huge thanks to **Chris Martin** for the detailed report. If you keep settings, keep them in `~/.nipei-os/config.json`.
...
- 🔢 **The dashboard now shows its build version.** Look under the "Nipei OS" logo (top-left) — it shows `build <date>`. Now you can tell at a glance whether you're on the latest pack; if it's older than the newest in the AI Profit Boardroom, run `Update Nipei OS.command`.
...
## 2026-06-20
- 🔧 **Your install is now truly YOURS** — fixed a bug where a fresh Nipei OS acted like it belonged to the creator: the Agent Room/Mastermind called you "Julian" and tried to reference his projects, and a few defaults pointed at his company and voice. Now every install is clean and personal — it uses **your** name, **your** vault, **your** company, **your** voice. Set your display name with `"userName"` in `~/.nipei-os/config.json` (defaults to "You"). *(Thanks Lenard for the heads-up!)*
...
## 2026-06-11
- ✨ **One-click updater** — double-click `Update Nipei OS.command` to move to the newest version. It keeps all your settings, keys, and notes. No terminal, no re-install.
...
## 2026-06-10
- ✨ **Double-click to start** — `Start Nipei OS.command` installs everything the first time and opens your dashboard. `Check My Setup.command` is a friendly health check that tells you what's set up and what isn't.
...
*Want the very latest? Download the newest pack from the AI Profit Boardroom, then double-click `Update Nipei OS.command`.*
