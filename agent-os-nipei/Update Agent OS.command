#!/bin/bash
# ─────────────────────────────────────────────────────────────
#  UPDATE Nipei OS — double-click me to move to the newest version.
#
#  What it does, automatically:
#   • finds the new nipei-os-pack zip you downloaded
...
#   ~/.nipei-os, ~/.hermes, ~/.fcc, and your Obsidian vault.
# ─────────────────────────────────────────────────────────────
...
[ -d "$APP" ] || { echo "  ❌ Can't find the 'source' folder next to this file. Put this file inside your Nipei OS folder."; read -p "  Press Enter."; exit 1; }
...
if [ -n "$AUTO_URL" ]; then
  ...
  NEWZIP="$(ls -t \
    "$HOME/Downloads/nipei-os-pack"*.zip \
    "$HOME/Desktop/nipei-os-pack"*.zip \
    "$HERE/../nipei-os-pack"*.zip 2>/dev/null | head -1)"
  if [ -z "$NEWZIP" ]; then
    echo "  📥 I couldn't find a new 'nipei-os-pack' zip in your Downloads."
    echo ""
...
[ -d "$NEWSRC" ] && [ -f "$NEWSRC/package.json" ] || { echo "  ❌ That zip doesn't look like an Nipei OS pack. Re-download and try again."; exit 1; }
