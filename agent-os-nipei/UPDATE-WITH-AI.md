# UPDATE-WITH-AI — Safe Nipei OS Updater (for Codex / Claude / Cursor, any OS)

**You are an AI coding agent (Codex, Claude Code, Cursor, Gemini, etc.). The user wants you to update their existing Nipei OS to a newer pack — safely, on macOS, Linux, OR Windows. Follow this playbook top to bottom. They are likely NOT technical.**

This is the cross-platform, agent-driven equivalent of `Update Nipei OS.command` (which is macOS-only). It works anywhere you can run a shell.
...
- `~/.nipei-os/` — settings (`config.json`: vault path, `userName`, model routing, keys-config)
...
1. Find the **current Nipei OS app folder** — the directory that contains `package.json` + a `source`/app you run with `npm start` (often `~/Nipei OS/nipei-os`, or wherever the user runs it). If unsure, ask.
2. Find the **newest `nipei-os-pack*.zip`** — usually in `~/Downloads`. Pick the one with the latest date in its name.
...
- Read the new version: unzip just `nipei-os/VERSION` from the pack zip.
...
Extract the zip to a temp folder. The new app code is at `<temp>/nipei-os/source/`.
...
- **macOS / Linux:**
  ...
  rsync -a --delete --exclude node_modules --exclude .next \
    --exclude '.env' --exclude '.env.*' --exclude 'nipei-os.config.json' --exclude '.git' \
    "<temp>/nipei-os/source/" "<app>/"
  ```
- **Windows (PowerShell):**
  ```powershell
  robocopy "<temp>\nipei-os\source" "<app>" /MIR /XD node_modules .next .git /XF .env .env.local ".env.*" nipei-os.config.json
  ```
  *(`/MIR` mirrors and removes app files no longer in the new version — the Windows equivalent of `--delete`. **`/XF` + `/XD` are ESSENTIAL: they stop the mirror deleting the user's `.env` / `.env.local` / `nipei-os.config.json` if any live inside the app folder.** The Step-2 backup is your safety net.)*
...
## Make it a one-word trigger (`$nipei-os-update`)
So the user can just say `$nipei-os-update` next time:
- **Claude Code:** save this file as a skill — copy it to `~/.claude/skills/nipei-os-update/SKILL.md` (add a one-line frontmatter `description:`), then invoke `/nipei-os-update`.
- **Codex / Cursor / other agents:** add a saved prompt/command that says *"Follow UPDATE-WITH-AI.md in my Nipei OS folder to update safely."* Trigger it with your agent's command shortcut.
- **Any agent, no setup:** just paste — *"Read `UPDATE-WITH-AI.md` and update my Nipei OS safely. Back up first, preserve my config/keys/vault, ask before anything risky."*
